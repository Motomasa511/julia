...
function process_matrix(data)
    # change 0 to NaN so that velocity=0 area isn't colored
...
function showwindow()
    ...
    # raylines
    n_rays = 25
    ray_indices = round.(Int, range(1, n_angles, length=n_rays))
    ...
    on(bt_reset.clicks) do _
        ...
        for i in 1:n_rays
            x_lines[i][] = [x[][ray_indices[i]]]
            y_lines[i][] = [y[][ray_indices[i]]]
            notify(x_lines[i])
            notify(y_lines[i])
        end        
    end
    ...
    Label(fig[8,12], "現在の速度分布:", fontsize=20, font = "Hiragino Kaku Gothic ProN")
    selected_label = Label(fig[9,12], "速度一様", fontsize=20, font = "Hiragino Kaku Gothic ProN")

    wave_speed_funcs = [wave_speed1, wave_speed2, wave_speed3, wave_speed4, wave_speed5, wave_speed6]
    labels = ["速度一様", "水平2層", "水平成層", "低速度領域", "速度分布5","異常震域"]

    for i in 1:6
        btn = Button(fig[7+i, 13], label=labels[i], font = "Hiragino Kaku Gothic ProN", width = 100, height = 40)
        on(btn.clicks) do _
            wave_speed_func[] = wave_speed_funcs[i]
            selected_label.text[] = labels[i]
            new_map = process_matrix(speedmap(n_points, wave_speed_func[]))
            hm[].values = new_map
        end
    end

    # switch wavefront, rays
    show_wavefront = Observable(true)
    show_rays = Observable(true)

    # switch button
    btn_color_wave = Observable(:orange)
    btn_color_ray = Observable(:orange)
    bt_wave = Button(fig[2, 13], label="波面 ON", width=150, buttoncolor=btn_color_wave)
    bt_rays = Button(fig[3, 13], label="波線 ON", width=150, buttoncolor=btn_color_ray)

    on(bt_wave.clicks) do _
        if show_wavefront[]
            btn_color_wave[] = :gray
        else
            btn_color_wave[] = :orange
        end
        show_wavefront[] = !show_wavefront[]
        bt_wave.label = show_wavefront[] ? "波面 ON" : "波面 OFF"
    end

    on(bt_rays.clicks) do _
        if show_wavefront[]
            btn_color_ray[] = :gray
        else
            btn_color_ray[] = :orange
        end
        show_rays[] = !show_rays[]
        bt_rays.label = show_rays[] ? "波線 ON" : "波線 OFF"
    end
    ...
    scatterplot = scatter!(ax1, raypoints, markersize=1.0, color="black", visible=lift(x -> x, show_wavefront))


    x_lines = [Observable([x[][i]]) for i in ray_indices]
    y_lines = [Observable([y[][i]]) for i in ray_indices]

    ray_lines = [scatter!(ax1, lift((xs, ys) -> Point2f.(xs, ys), x_lines[i], y_lines[i]);
                              color="gray", markersize=1.0, visible=show_rays) for i in 1:n_rays]
    ...
    while true
        if start[]
            ...
            for (i, idx) in enumerate(ray_indices)
                push!(x_lines[i][], x[][idx])
                push!(y_lines[i][], y[][idx])
                notify(x_lines[i])
                notify(y_lines[i])
            end
